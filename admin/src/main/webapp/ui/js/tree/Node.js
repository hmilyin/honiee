...
xyTree.Node.prototype.innerhtml = function() {
  ...
  img.onclick = function() { 
  	//alert("enter default img onclick!!!!");
    //他的影响是双向的，
    //对于底部，全部选中
    //对于上方，要考虑是部分选中还是全部选中，并且立即更改属性并绘制
    var fu = this.parentNode;
  	var parentyangshi = current.parent.checked;   
	  //alert("node_nomarl"+"状态"+current.checked);
	  //alert("node_nomarl"+"父节点状态"+parentyangshi);
    if (current.checked == 1 || current.checked == 2) {
		...
		current.upDigui(current);
		current.downDigui(current);
    } else if(current.checked == 3)return;
  }


  //////////////////
  img.onclickReadOnly = function(node) { 
  	//alert("enter onclickReadOnly---->name:"+node.name+"checked:"+node.checked);
    //他的影响是双向的，
    //对于底部，全部选中
    //对于上方，要考虑是部分选中还是全部选中，并且立即更改属性并绘制
    var fu = this.parentNode;
    if(node.checked==1){
    	this.src = xyTree.TreeConfig.quanzhongIcon;
    	//alert("onclickReadOnly---------->name:"+node.name+"checked:1");
    }else if(node.checked==3){
    	//alert("onclickReadOnly---------->name:"+node.name+"checked:3");
    	this.src = xyTree.TreeConfig.readonlyzhongIcon;
    }
    node.upDiguiReadOnly(node);
  }
  /////////////////
...
//////////////////////
/**
 * 确定复选框的状态，并向上递归   只有在初始化时使用
 * @param {xyTree.Node} node 对这个参数节点递归，调用对象节点没用
 */
xyTree.Node.prototype.upDiguiReadOnly = function(node) {
  if (node.level == 0) return;
  var parentyangshi = node.parent.checked;            //父节点样式，可能是0，1，2
  var yangshi = node.checked;          //获得当前节点的样式,记住不可能是局部，只会是0或1
  if(parentyangshi==0){
  	node.parent.checked=yangshi;
  	node.parent.getHtmlElementfuxuanimg().src = this.getimgbyyangshi(node.parent.checked); //这是改html样式
  	this.upDiguiReadOnly(node.parent);
  }else if(parentyangshi==1&&yangshi==3){
	  node.parent.checked=yangshi;
	  node.parent.getHtmlElementfuxuanimg().src = this.getimgbyyangshi(node.parent.checked); //这是改html样式
	  this.upDiguiReadOnly(node.parent);
  }
...
xyTree.Node.prototype.getimgbyyangshi = function(checked) {
  ...
  if (checked == 3) return xyTree.TreeConfig.readonlyzhongIcon;
}
